...
const TaskList = memo(({ showArchive = false }: TaskListProps) => {
  ...
  const getCachedUser = useCallback(async (userId: string) => {
    ...
    (async () => {
      ...
      const uniqueUserIds = [...new Set([
        ...contextFilteredTasks.map(t => t.assignedTo),
        ...contextFilteredTasks.map(t => t.createdBy)
      ].filter(Boolean))];
      
      // Предзагружаем всех пользователей одним запросом
      await Promise.all(uniqueUserIds.map(id => getCachedUser(id)));
      
      const tasksWithUsers = await Promise.all(contextFilteredTasks.map(async t => ({
        ...t, 
        assignee: await getCachedUser(t.assignedTo),
        creator: await getCachedUser(t.createdBy)
      })));
  ...
  }, [tasks, showArchive, user?.id, taskFilter, getFilteredTasks, selectedUserId, getCachedUser]);
