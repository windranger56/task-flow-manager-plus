...
export default function TaskDetail() {
  ...
  const fetchHistoryBackwards = async (task: Task) => {
    ...
    while (true) {
      ...
      tasks[0].assignee = await supabase
        .from("users")
        .select("fullname")
  ...
  const fetchHistoryForwards = async (task: Task) => {
    ...
    while (true) {
      ...
      tasks[tasks.length - 1].assignee = await supabase
        .from("users")
        .select("fullname")
        .eq("id", tasks[tasks.length - 1].assigned_to)
        .then(({ data }) => data[0]);
    }
        ...
        <div className="grid grid-cols-7 h-full">
          ...
          <div className="col-span-7 md:col-span-5 flex flex-col flex-1 overflow-auto p-8 border-r-2 border-[#E1E3E6] h-full relative">
            ...
            <div className="flex flex-row lg:flex-col gap-2 mb-6">
              ...
              <h1 className="text-2xl text-[#020817] font-bold">
                {selectedTask.title}
              </h1>
            </div>
            {/* Описание поручения */}
            ...
            <div className="w-full mt-8 pt-4 pb-16">
              ...
              <div className="flex justify-end mb-2 px-4 text-[#757D8A] font-thin">
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    setShowOnlySystemMessages(!showOnlySystemMessages)
                  }
                ...
                {filteredMessages.map((msg) => (
                  ...
                  >
                    {msg.is_deleted ? (
                      ...
                      <>
                        <div
                          className={`break-all whitespace-pre-wrap ${msg.is_system ? "w-full text-center" : ""}`}
                        >
                          {msg.content}
                          ...
                          {msg.files &&
                            msg.files.map(
                              (
                                ...
                                <div
                                  key={index}
                                  className="mt-2 p-2 border border-gray-200 rounded bg-blue-100"
                                >
          ...
          <div className="hidden md:flex col-span-2 flex-col px-4 py-8 gap-12">
            <div className="w-full flex flex-col gap-2 items-center">
              ...
              <Avatar className="h-[100px] w-[100px]">
                ...
                <AvatarFallback>
                  {assignee?.fullname?.slice(0, 2)}
                </AvatarFallback>
              </Avatar>
              <p className="max-w-[70%] text-center text-[#020817] text-xl line-height-[140%] font-normal uppercase">
                {assignee?.fullname}
              </p>
                      ...
                      <div className="flex flex-col gap-2 mt-2">
                        ...
                        {getAvailableStatuses().length === 0 && (
                          <span className="text-gray-500 text-sm">
                            Нет доступных статусов для смены
                          </span>
                        )}
                      </div>
                    </div>
                    ...
                    {user.id === selectedTask.createdBy && (
                      <div className="mb-4">
                        ...
                        <Input
                          type="date"
                          value={
                            newDeadline ? format(newDeadline, "yyyy-MM-dd") : ""
                          }
                          onChange={(e) =>
                            setNewDeadline(
                              e.target.value
                            ...
                            return (
                              ...
                              >
                                <div className="flex items-start gap-3">
                                  <div
                                    className={`h-8 w-8 rounded-full flex items-center justify-center ${isCurrentTask ? "bg-blue-500 text-white" : "bg-gray-300 text-gray-600"}`}
                                  >
                                    {index + 1}
                                  </div>
                                  <div className="flex-1">
                                    <div className="flex items-center justify-between">
                                      <h4
                                        className={`font-medium ${isCurrentTask ? "text-blue-600" : "text-gray-700"}`}
                                      >
                                        {task.title}
                                      </h4>
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={reassignTo} onValueChange={setReassignTo}>
                        ...
                        <SelectContent>
                          {subordinates
                            .filter(
                              (user) => user.id !== selectedTask.assignedTo,
                            )
                            .map((user) => (
          ...
          <Dialog open={showReassign} onOpenChange={setShowReassign}>
            <DialogContent>
              ...
              <div className="space-y-4 py-4">
                <div className="space-y-2">
                  <Label htmlFor="reassign-to">
                    Переназначить <span className="text-red-500">*</span>{" "}
                  </Label>
                  <Select value={reassignTo} onValueChange={setReassignTo}>
                    <SelectTrigger>
                      <SelectValue placeholder="Выберите сотрудника" />
