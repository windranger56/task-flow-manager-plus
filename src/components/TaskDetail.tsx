...
export default function TaskDetail() {
  ...
  function getAvailableStatuses(): Array<{ value: TaskStatus; label: string }> {
    ...
    if (status === 'new' && isExecutor) {
      return [{ value: 'in_progress' as TaskStatus, label: 'В работе' }];
    }
    if (status === 'in_progress' && isExecutor) {
      return [{ value: 'on_verification' as TaskStatus, label: 'На проверке' }];
    }
    if (status === 'on_verification' && isCreator) {
      return [
        { value: 'completed' as TaskStatus, label: 'Завершено' },
        { value: 'in_progress' as TaskStatus, label: 'В работе (вернуть на доработку)' },
      ];
    }
    if (status === 'overdue' && isCreator) {
      return [
        { value: 'in_progress' as TaskStatus, label: 'В работе' },
        { value: 'on_verification' as TaskStatus, label: 'На проверке' },
      ];
    }
    return [];
  }
							...
							<Dialog open={!!selectedImage} onOpenChange={(open) => !open && setSelectedImage(null)}>
								<DialogContent className="max-w-[90vw] max-h-[90vh] flex items-center justify-center">
									<VisuallyHidden>
										...
										<DialogDescription>Увеличенное изображение из сообщения</DialogDescription>
									</VisuallyHidden>
									<img 
										src={selectedImage || ''} 
					...
					<div className='hidden md:flex col-span-2 flex-col px-4 py-8 gap-12'>
						...
						<div className='w-full flex flex-col gap-4 items-center'>
							...
							<button
								...
								onClick={() => {
									console.log('🎨 Desktop button clicked, status:', selectedTask.status);
							...
							<DialogContent>
								<DialogHeader>
									...
									<DialogDescription>Выберите новый статус для поручения</DialogDescription>
								</DialogHeader>
								<div className="py-4">
									<div className="mb-4">
							...
							<Dialog open={historyOpen} onOpenChange={setHistoryOpen}>
								<DialogContent className={cn(
									...
									<DialogHeader>
										...
										<DialogDescription>Хронология изменений и событий по поручению</DialogDescription>
									</DialogHeader>
							...
							<Dialog open={showReassign} onOpenChange={setShowReassign}>
								<DialogContent>
									<DialogHeader>
										...
										<DialogDescription>Выберите нового исполнителя для поручения</DialogDescription>
									</DialogHeader>
			...
			<div className='w-full h-full flex md:hidden flex-col items-center gap-6 bg-[#f7f7f7] p-4 pb-32 overflow-y-auto'>
				...
				<div className='relative bg-white shadow-md rounded-xl min-h-72 w-full flex flex-col overflow-y-auto p-[10px]'>
					...
					<div className="h-full overflow-y-auto mb-4 rounded-md pb-16">
						...
						>
							...
							{/* Нижняя часть сообщения с временем и кнопкой удаления */}
				...
				<button
					...
					onClick={(e) => {
						...
						console.log('🎨 Mobile button clicked, status:', selectedTask.status);
						console.log('📊 Available statuses:', getAvailableStatuses());
						console.log('🔍 About to open modal from TaskDetail');
						// Открываем модальное окно через контекст
						setIsMobileStatusModalOpen(true);
						console.log('✅ Mobile modal state set to true from TaskDetail');
					}}
				>
					{iconFromStatus[selectedTask.status]}
					{getStatusLabel(selectedTask.status)}
