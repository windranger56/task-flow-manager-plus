...
const LeftSidebar = memo(({ onItemClick }: LeftSidebarProps) => {
  const {
    user,
    setUser,
    departments,
  ...
  useEffect(() => {
    if (user) {
      setDoneTasks(tasksByStatus.done);
      setOverdueTasks(tasksByStatus.overdue);
      setNewTasks(tasksByStatus.new);
      setinworkTasks(tasksByStatus.inwork);
      setverifyTasks(tasksByStatus.verify);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      fetchTasksWithNewMessages();
      fetchTasksWithNewStatus();
    }, 60000); // Refresh every 60 seconds (optimized from 30s)
